...
'''
    ...
    assert res["min_ms"] == 14.810
    assert res["avg_ms"] == 14.955
    assert res["max_ms"] == 15.120
    assert res["mdev_ms"] == 0.145

def test_parse_ping_output_loss():
    stdout = '''
...
'''
    res = parse_ping_output(stdout)
    assert res["loss_percent"] == 75.0
    assert res["received"] == 1

def test_parse_mtr_json():
    stdout = '''{
        "report": {
            "mtr": {
                "src": "moonshield",
    ...
    assert res["hops"][0]["ip"] == "192.168.1.1"
    assert res["hops"][0]["loss_percent"] == 0.0

def test_parse_ip_j():
    stdout = '[{"ifindex": 1, "ifname": "lo"}]'
    res = parse_ip_j(stdout)
    assert len(res) == 1
    assert res[0]["ifname"] == "lo"
