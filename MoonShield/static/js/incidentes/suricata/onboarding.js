...
document.addEventListener('DOMContentLoaded', () => {
  ...
  async function loadEnvironment(force = false) {
    ...
    try {
      ...
      const suricata = stack.suricata || data.suricata || {};
      const services = stack.servicos || data.servicos || {};
      const paths = suricata.yaml || data.caminhos || environment.caminhos || {};

      const linux = readBoolean(environment, ['linux', 'eh_linux', 'is_linux'], false);
      const root = readBoolean(environment, ['root', 'privilegios', 'privilegiado', 'is_root'], false);
      const systemd = readBoolean(environment, ['systemd', 'tem_systemd'], readBoolean(services, ['disponivel'], false));
      ...
      const pathReady = Boolean(paths.existe ?? paths.yaml_existe ?? suricata.yaml?.existe ?? installed);

      const checks = {
        linux: {
          ok: linux,
          warning: !linux,
          value: linux ? firstText(environment.distribuicao, environment.sistema, 'Linux detectado') : 'Linux necessário',
        },
        privilegios: {
          ok: root,
          warning: !root,
