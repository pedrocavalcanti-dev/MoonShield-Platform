(() => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const risk = (item) => item.risk === null ? '<span class="dev-cell-dim">-</span>' : `<span class="dev-risk dev-risk--${item.risk >= 70 ? "high" : item.risk >= 40 ? "medium" : "low"}">${escapeHtml(item.risk)}</span>`;
    ...
    function openDrawer(item) {
      ...
      const icon = $("drawerTypeIcon"); if (icon) icon.className = `bi ${iconFor(item.type)} drawer-type-icon`; ["dStatDNS","dStatBlock","dStatSOC","dStatFW","dStatRPM"].forEach((id) => set(id,"-"));
      ...
      ["drawerFlagNew","drawerFlagMal"].forEach((id) => { const flag=$(id); if(flag) flag.style.display="none"; }); $("devDrawer")?.classList.add("dev-drawer--open"); $("devDrawerOverlay")?.classList.add("dev-drawer-overlay--show");
    }
    const closeDrawer = () => { $("devDrawer")?.classList.remove("dev-drawer--open"); $("devDrawerOverlay")?.classList.remove("dev-drawer-overlay--show"); };
    const closeRename = () => $("renameModalOverlay")?.classList.remove("dev-modal-overlay--show");
    function openRename(item = selected()) { if (!item) return; state.selectedId=item.id; set("renameModalCurrent",item.displayName); set("renameModalSub",text(item.ip)); const input=$("renameInput"); if(input){input.value=item.displayName; set("renameCharCount",`${input.value.length}/80`); input.focus();} $("renameModalOverlay")?.classList.add("dev-modal-overlay--show"); }
    ...
    tbody?.addEventListener("click",(event)=>{const action=event.target.closest("[data-action]")?.dataset.action;if(action==="scan"){document.dispatchEvent(new CustomEvent("moonshield:scan:open"));return;}const item=state.devices.find((device)=>device.id===event.target.closest("tr[data-device-id]")?.dataset.deviceId);if(action==="open")openDrawer(item);if(action==="rename")openRename(item);}); $("devDrawerClose")?.addEventListener("click",closeDrawer); $("devDrawerOverlay")?.addEventListener("click",closeDrawer); $("daBtnRename")?.addEventListener("click",()=>openRename()); $("renameModalClose")?.addEventListener("click",closeRename); $("renameModalCancel")?.addEventListener("click",closeRename); $("renameModalOverlay")?.addEventListener("click",(event)=>{if(event.target===event.currentTarget)closeRename();}); $("renameInput")?.addEventListener("input",(event)=>set("renameCharCount",`${event.target.value.length}/80`)); $("renameModalSave")?.addEventListener("click",saveRename);
