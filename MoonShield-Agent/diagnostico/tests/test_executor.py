...
def test_dispatch_ping_success(mock_run, mock_get_bin):
...
rtt min/avg/max/mdev = 1.0/2.0/3.0/0.1 ms'''
    ...
    assert result["structured"]["sent"] == 4
    assert result["structured"]["received"] == 4
    assert result["structured"]["loss_percent"] == 0.0

@patch("diagnostico.executor._get_bin")
def test_dispatch_tool_not_allowed(mock_get_bin):
    result = dispatch_tool("rm", "/", {})
    assert result["ok"] is False
    assert result["error_code"] == "tool_not_allowed"

@patch("diagnostico.executor._get_bin")
def test_dispatch_tool_unavailable(mock_get_bin):
...
def test_tcp_connect_mock(mock_socket):
    ...
    assert result["ok"] is True
    assert result["status"] == "ok"
    assert result["structured"]["reachable"] is True
    assert result["structured"]["port"] == 80
    mock_sock_inst.connect.assert_called_with(("8.8.8.8", 80))

@patch("diagnostico.executor.urllib.request.urlopen")
def test_http_check_mock(mock_urlopen):
    mock_resp = MagicMock()
